...
public:
    ...
    std::unordered_map<std::string, double> calculatePageRank() {
        ...
        std::unordered_map<std::string, std::vector<std::string>> inEdges; // 反向邻接表
        std::unordered_map<std::string, int> outDegree;
        // 初始化PR值和出度
        double initialPr = 1.0 / adjList.size();
        for (const auto& node : adjList) {
            pr[node.first] = initialPr;     // 初始PR值为1/N
            outDegree[node.first] = node.second.size();   // 计算每个节点的出度
        }
        // 构建反向邻接表
        for (const auto& src : adjList) {
            for (const auto& dest : src.second) {
                inEdges[dest.first].push_back(src.first);  // 创建从目标节点到源节点的反向映射
            }
        }
        // 迭代计算：[ 核心公式：PR(u) = (1-d)/N + d * Σ(PR(v)/L(v)) ]
        for (int iter = 0; iter < MAX_ITER; ++iter) {
            std::unordered_map<std::string, double> newPr;
            double diff = 0.0;
            // 计算每个节点的新PR值
            for (const auto& node : pr) {
                ...
                for (const std::string& v : inEdges[u]) {
                    sum += pr[v] / outDegree[v];
                }
                // 完整PR公式
                newPr[u] = (1.0 - DAMPING) / adjList.size() + DAMPING * sum;
            }
            // 计算PR值变化量并检查收敛
            for (const auto& node : pr) {
                diff += std::abs(newPr[node.first] - pr[node.first]);
            }
            pr = newPr;
            if (diff < TOLERANCE) break;    // 如果变化很小，提前终止
        }
        return pr;
    }
